#!/usr/bin/env python3
# -*- coding: utf-8 -*-
from openpasen import api, common
from openpasen.extra import reporte
from openpasen.extra import horario as genhorario
import gi
gi.require_version('Gtk', '3.0')
from gi.repository import Gtk
from openpasen.notebook import Notebook as main_notebook
...
class Handler:
    ...
    def on_login_clicked(self, button):
        ...
        if (user["tipo"] == "TUT_LEGAL"):
            builder.get_object("tutorlegal_menu").show()
        else:
    ...
    def on_tutorlegal_menu_show(self, *args):
        ...
        #for i in range(0,len(user['hijos'])):
        #    pass
    # -- Main menu -- #
    ...
    def on_centro_activate(self, button):
        centro = api.centro()
        centro_menu = builder.get_object("centro_menu")

        tree = builder.get_object("centro_treeview")
        store = builder.get_object("centro_store")
        store.clear()
        for info in centro:
    ...
    def on_horario_gen_activate(self, button):
        ...
        if out not in "ImportError":
            builder.get_object("general_label").set_markup(f'Se ha generado el horario en el directorio:\n{out}')
        else:
            builder.get_object("general_label").set_markup("Ha habido un error al generar el reporte")
    # Cerrar sesión
